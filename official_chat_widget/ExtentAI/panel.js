...
// ============================================================
// CHART RENDERING (Native Plotly parsing)
// ============================================================
function renderChart(content) {
    if (typeof content !== "string") return;
    const container = document.createElement("div");
    container.style.cssText = "margin:8px 0;border-radius:10px;overflow:hidden;border:1px solid #e2e8f0;background:#fff;padding:8px;";

    function processPlotlyHtml(html) {
        try {
            // Regex to extract JSON arguments from Plotly.newPlot("id", [data], {layout}, {config})
            const match = html.match(/Plotly\.newPlot\(\s*['"][^'"]+['"]\s*,\s*(\[.*?\])\s*,\s*(\{.*?\})\s*(?:,\s*(\{.*?\})\s*)?\)/s);
            if (!match) throw new Error("Không tìm thấy bộ dữ liệu Plotly trong gói tải về.");
            
            const [ , dataStr, layoutStr, configStr ] = match;
            const data = JSON.parse(dataStr);
            const layout = JSON.parse(layoutStr);
            const config = configStr ? JSON.parse(configStr) : undefined;
            
            // === AUTO-RESIZE & COMPACT LAYOUT FOR EXTENSION ===
            delete layout.width;
            delete layout.height;
            layout.margin = { l: 30, r: 10, t: 30, b: 20 }; // Very compact margins
            if (layout.title && typeof layout.title === 'string') {
                layout.title = { text: layout.title, font: { size: 12 } };
            } else if (layout.title && layout.title.font) {
                layout.title.font.size = 12;
            }
            if (layout.legend) {
                layout.legend.orientation = "h";
                layout.legend.y = -0.2; // Move legend below the chart
            }
            
            // Generate unique div for Plotly
            const uid = "chart-" + Math.random().toString(36).substr(2, 9);
            ...
            container.innerHTML = "";
            container.style.position = "relative"; // Setup for absolute positioning
            container.appendChild(targetDiv);
            
            // Phóng to button overlay
            const expandBtn = document.createElement("button");
            expandBtn.innerHTML = "⛶ Xem lớn";
            expandBtn.title = "Mở biểu đồ trong tab mới rông hơn";
            expandBtn.style.cssText = "position:absolute; top:4px; right:4px; z-index:100; background:rgba(255,255,255,0.9); border:1px solid #cbd5e1; padding:4px 8px; border-radius:6px; font-size:11px; color:#334155; font-weight:600; cursor:pointer;";
            expandBtn.onmouseover = () => { expandBtn.style.background = "#f1f5f9"; };
            expandBtn.onmouseout = () => { expandBtn.style.background = "rgba(255,255,255,0.9)"; };
            expandBtn.onclick = () => {
            ...
            // Check if local plotly.min.js loaded successfully
            if (window.Plotly) {
                // Config to hide toolbar and make responsive
                const plotConfig = config || {};
                plotConfig.displayModeBar = false;
                plotConfig.responsive = true;
                
                Plotly.newPlot(uid, data, layout, plotConfig).then(() => {
                    // Smoothly handle resize events when the chat panel width changes
                    const ro = new ResizeObserver((entries) => {
                        for (let entry of entries) {
                            if (entry.contentRect.width > 0) {
                                // Force exact width update overriding old SVG bounds
                                Plotly.relayout(targetDiv, { width: entry.contentRect.width });
                            }
                        }
                    });
                ...
                container.innerHTML = '<div style="padding:12px;color:#b91c1c;font-size:12px;">❌ Thư viện Plotly chưa tải xong. Quý khách vui lòng thử lại sau.</div>';
            }
        } catch (e) {
            console.error("[chart] Phân tích biểu đồ lỗi:", e);
            container.innerHTML = '<div style="padding:12px;color:#b91c1c;font-size:12px;">❌ Lỗi vẽ biểu đồ dữ liệu</div>';
        }
    }

    if (content.startsWith("http")) {
        container.innerHTML = '<div style="height:200px;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:13px;">📊 Đang xử lý...</div>';
        botElement(container);
        fetch(content)
            .then(r => r.text())
            .then(processPlotlyHtml)
            .catch(() => {
                container.innerHTML = '<div style="padding:12px;color:#b91c1c;font-size:12px;">❌ Không thể kết nối biểu đồ</div>';
            });
    } else if (content.includes("<html")) {
        container.innerHTML = '<div style="height:200px;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:13px;">📊 Đang xử lý...</div>';
        botElement(container);
        setTimeout(() => processPlotlyHtml(content), 50);
    }
}
