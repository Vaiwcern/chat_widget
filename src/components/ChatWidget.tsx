...
import { SendHorizontal, Maximize2, X, FileText, Download } from 'lucide-react';
...
export default function ChatWidget() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'Xin chào! Tôi là Bé Gạo 🍚 Mình có thể giúp gì cho bạn?',
      sender: 'bot',
      timestamp: new Date(),
    },
  ]);
  ...
  const renderMessageContent = (message: Message) => {
    if (message.type === 'table' && message.content) {
      return (
        <div
          className="mt-2 bg-slate-900 p-2 rounded text-xs overflow-x-auto border border-slate-600"
          dangerouslySetInnerHTML={{
            __html: message.content
              .replace(/border="1"/g, '')
              .replace(/<table/g, '<table style="border-collapse: collapse; width: 100%;"')
              .replace(/<th/g, '<th style="border: 1px solid #64748b; padding: 6px 8px; background-color: #334155; text-align: left;"')
              .replace(/<td/g, '<td style="border: 1px solid #64748b; padding: 6px 8px;"')
          }}
        />
      );
    }
      ...
      if (message.content.includes('<html')) {
        return (
          <div className="mt-2 rounded-lg overflow-hidden border border-slate-600 w-full bg-white relative group shrink-0">
            ...
            />
            <div
              className="absolute inset-0 bg-slate-900/10 hover:bg-slate-900/30 transition-colors flex items-center justify-center cursor-pointer opacity-0 group-hover:opacity-100"
              onClick={() => setExpandedChart(message.content)}
            >
              <span className="bg-slate-900 text-white px-3 py-1.5 rounded-md text-xs flex items-center gap-1.5 shadow-lg font-medium transform scale-95 group-hover:scale-100 transition-transform">
        ...
        return (
          <div className="mt-2 rounded-lg border border-slate-600 w-full h-[250px] bg-slate-800 flex items-center justify-center shrink-0">
            {message.content.startsWith('http') ? (
              <div className="flex flex-col items-center gap-3">
                <div className="h-5 w-5 rounded-full border-2 border-slate-400 border-t-cyan-400 animate-spin"></div>
                <span className="text-xs text-slate-400">Đang tải biểu đồ...</span>
              </div>
            ) : (
              <span className="text-xs text-slate-500">Lỗi hình ảnh</span>
            )}
          </div>
        );
      }
    ...
    if (message.type === 'file' && message.content) {
      ...
      const colorScheme = isPdf
        ? 'text-red-500 bg-red-100/10 border-red-500/30'
        : isExcel
          ? 'text-emerald-500 bg-emerald-100/10 border-emerald-500/30'
          : isWord
            ? 'text-blue-500 bg-blue-100/10 border-blue-500/30'
            : 'text-slate-400 bg-slate-100/10 border-slate-500/30';
      ...
      return (
        <div className="mt-2 w-full max-w-[280px]">
          <a
            ...
            className="flex items-center gap-3.5 p-3 bg-slate-800 rounded-xl border border-slate-700 hover:bg-slate-700/80 transition-colors group no-underline shadow-sm"
          >
            ...
            <div className={`relative flex flex-col justify-center items-center w-11 h-14 rounded shadow-sm shrink-0 border ${colorScheme}`}>
              {/* Folded corner layer */}
              <div className={`absolute top-0 right-0 w-3.5 h-3.5 bg-slate-800 rounded-bl border-b border-l ${cornerSplit}`}></div>
              <span className="text-[10px] font-black tracking-tighter uppercase mt-2">{ext.slice(0, 4)}</span>
            </div>

            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <p className="text-sm font-semibold text-slate-100 truncate group-hover:text-white transition-colors" title={filename}>
                {filename}
              </p>
            ...
            <div className="flex-shrink-0 text-slate-400 group-hover:text-white transition-colors bg-slate-700/50 group-hover:bg-slate-600 p-2 rounded-full cursor-pointer border border-slate-600/50">
  ...
  return (
    ...
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end gap-3 pointer-events-none">
      ...
      {/* Chat Widget Card */}
      {isOpen && (
        <Card className="w-[340px] sm:w-[380px] p-0 gap-0 flex flex-col shadow-2xl shadow-blue-900/20 border-slate-700 bg-slate-950 rounded-2xl overflow-hidden pointer-events-auto origin-bottom-right animate-in zoom-in slide-in-from-bottom-2 duration-200" style={{ height: '600px', maxHeight: 'calc(100vh - 120px)' }}>
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-4 py-3 border-b border-cyan-500/20 flex items-center justify-between shadow-sm shrink-0">
              ...
              {isLoading && (
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-700">
                      <div className="flex gap-1">
                        <div className="h-1.5 w-1.5 rounded-full bg-slate-500 animate-bounce"></div>
                        <div className="h-1.5 w-1.5 rounded-full bg-slate-500 animate-bounce animation-delay-100"></div>
                        <div className="h-1.5 w-1.5 rounded-full bg-slate-500 animate-bounce animation-delay-200"></div>
                      </div>
                    </div>
                    {currentNotification && (
                      <div className="px-3 py-1 rounded-lg bg-slate-700 text-slate-300 text-xs italic">
                        {currentNotification}
                      </div>
                    )}
                  </div>
          ...
          {recommendQuestions.length > 0 && (
            <div className="px-3 py-1.5 border-t border-slate-700 bg-slate-900/50">
              <p className="text-xs text-slate-400 mb-1.5">💡 Gợi ý:</p>
              <div className="grid grid-cols-2 gap-1">
                {recommendQuestions.map((question, index) => (
                  <button
                    ...
                    className="text-left px-2 py-1 rounded text-xs text-slate-100 bg-slate-800 hover:bg-slate-700 transition-colors border border-slate-700 hover:border-cyan-500 truncate"
                  >
                    {question}
                  </button>
                ))}
          ...
          {/* Input Area */}
          <div className="border-t border-slate-700 bg-slate-900 px-3 py-2 rounded-b-none sm:rounded-b-lg">
            <div className="flex gap-2">
              <Input
                value={input}
                ...
                className="flex-1 bg-slate-800 border-slate-600 text-white placeholder-slate-500 focus:border-cyan-500 text-sm h-9"
      ...
      <button
        ...
        className="h-14 w-14 rounded-full shadow-2xl shadow-blue-900/40 border-2 border-slate-700 bg-slate-800 hover:bg-slate-700 hover:scale-105 transition-all overflow-hidden flex items-center justify-center pointer-events-auto shrink-0 relative group focus:outline-none"
      >
        {isOpen ? (
          <X className="text-slate-300 group-hover:text-white w-6 h-6 transition-colors" />
        ) : (
